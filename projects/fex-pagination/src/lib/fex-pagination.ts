...
@Component({
  ...
  selector: 'fex-pagination',
  styleUrl: './fex-pagination.css',
  templateUrl: './fex-pagination.html',
})
export class FexPagination {
  pages = input(0);
  ...
  previewPageEvent = output<string | number>();
  ...
  nextPageSelected(numberPage: string | number) {
    this.activePage.set(+numberPage);
  ...
  nextPagePrefet(numberPage: string | number) {
    this.previewPageEvent.emit(+numberPage);
  }
}
